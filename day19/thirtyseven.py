...
def main() -> int:
    ...
    with open('input.txt', 'r') as f_in:
        workflows, parts = f_in.read().split('\n\n')
    workflows_list = splitlines(workflows)
    # workflows = {  # TODO could use regEx
    #     workflow_split[0]: {
    #         'conditions': [],
    #         True: workflow_split[1].split(',')[0].split(':')[1],
    #         False: workflow_split[1].rstrip('}').split(',')[-1],
    #     }
    #     for workflow in workflows_list
    #     if (workflow_split := workflow.split('{'))  # finally usecase for walrus :)
    # }
    # TODO could be faster if we ignore conditions on workflows that have the same True/False result
    workflows = {}
    for workflow in workflows_list:
        workflow_split = workflow.split('{')
        workflows[workflow_split[0]] = []
        for condition in workflow_split[1].split(',')[:-1]:
    ...
    for part in parts:
        ...
        while state not in (ACCEPT, REJECT):
            ...
            for count, condition in enumerate(workflows[state]):
                value = part[condition['category']] in condition['condition']
                if not value and count + 1 != len_condition:
                    continue
